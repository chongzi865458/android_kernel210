/*
 * linux/drivers/char/fl210_pwm.c
 *
...
static int fl210_pwm_open(struct inode *inode, struct file *file) {
...
static int fl210_pwm_close(struct inode *inode, struct file *file) {
	up(&lock);
	return 0;
}

static long fl210_pwm_ioctl(struct file *filep, unsigned int cmd,
		unsigned long arg)
{
	switch (cmd) {
...
static struct file_operations fl210_pwm_ops = {
	.owner			= THIS_MODULE,
	.open			= fl210_pwm_open,
	.release		= fl210_pwm_close, 
	.unlocked_ioctl	= fl210_pwm_ioctl,
};

static struct miscdevice fl210_misc_dev = {
	.minor = MISC_DYNAMIC_MINOR,
	.name = DEVICE_NAME,
	.fops = &fl210_pwm_ops,
};

static int __init fl210_pwm_dev_init(void) {
	int ret;
	...
	init_MUTEX(&lock);
	ret = misc_register(&fl210_misc_dev);
...
static void __exit fl210_pwm_dev_exit(void) {
	pwm_stop();

	misc_deregister(&fl210_misc_dev);
}

module_init(fl210_pwm_dev_init);
module_exit(fl210_pwm_dev_exit);
